    ...
    <>
      <header className="bg-white/90 backdrop-blur-xl shadow-sm border-b border-white/20 sticky top-0 z-50 transition-all duration-300">
        <div className="container mx-auto px-4 py-3 sm:py-4 flex justify-between items-center relative gap-4">
          ...
          <div className="flex items-center gap-3 sm:gap-4 shrink-0 justify-start">
            ...
            <Link to="/" className="flex items-center hover:scale-105 transition-transform shrink-0">
              {globalSettings?.storeLogo ? (
                <img src={globalSettings.storeLogo} alt="Store Logo" className="h-8 sm:h-10 lg:h-12 w-auto object-contain drop-shadow-sm" />
              ) : (
                <span className="text-2xl sm:text-3xl lg:text-4xl font-heading font-black italic tracking-tighter text-dark">
          ...
          <div className="hidden md:flex justify-center items-center flex-1 px-2 lg:px-6 z-20">
            <div className="flex w-full md:max-w-md lg:max-w-2xl bg-gray-100/70 rounded-2xl border-2 border-transparent focus-within:border-primary focus-within:bg-white focus-within:ring-4 focus-within:ring-primary/10 transition-all duration-300 shadow-inner relative h-12">
              ...
              <CustomSelect
                ...
                triggerClassName="group !border-0 !bg-transparent !py-0 !px-4 border-e border-gray-200 !rounded-s-2xl !rounded-e-none font-heading text-sm font-bold text-gray-600 hover:text-primary uppercase h-full tracking-wide transition-colors"
                menuClassName="!w-56"
                placeholder={t('header.all_categories')}
                renderValue={(opt) => (
                  <span className="truncate max-w-20 xl:max-w-30 font-heading tracking-wide uppercase text-sm font-bold text-gray-600 group-hover:text-primary transition-colors">
                    {opt?.label || t('header.all_categories')}
                  </span>
                )}
              />
          ...
          <div className="flex items-center justify-end gap-3 sm:gap-5 shrink-0">
            ...
            {userInfo ? (
              <div className="relative hidden md:block" ref={userDropdownRef}>
                <button onClick={() => setIsUserDropdownOpen(!isUserDropdownOpen)} className="flex items-center gap-2 text-dark font-bold hover:text-primary transition-colors focus:outline-none py-1 whitespace-nowrap cursor-pointer group">
                  <FaUserCircle className="text-2xl text-gray-400 group-hover:text-primary transition-colors" />
                  <span className="truncate max-w-25 text-base font-heading uppercase tracking-wide group-hover:text-primary transition-colors">{userInfo.name.split(' ')[0]}</span>
                ...
                <div className={`absolute z-50 inset-e-0 mt-3 w-56 bg-white/95 backdrop-blur-lg rounded-2xl shadow-2xl border border-gray-100 transition-all duration-300 transform origin-top-right rtl:origin-top-left ${ isUserDropdownOpen ? 'opacity-100 scale-100 visible' : 'opacity-0 scale-95 invisible' }`}>
                  <div className="py-2 text-start">
                    ...
                    <Link to="/profile" onClick={() => setIsUserDropdownOpen(false)} className="flex items-center gap-3 px-4 py-3 text-sm font-bold text-gray-600 hover:bg-primary/5 hover:text-primary transition-colors rounded-xl mx-1.5 group">
                      <FaUser className="text-gray-400 group-hover:text-primary transition-colors text-lg" /> {t('header.my_account')}
                    </Link>
                    {userInfo.isAdmin && (
                      <>
                        <Link to="/admin/dashboard" onClick={() => setIsUserDropdownOpen(false)} className="flex items-center gap-3 px-4 py-3 text-sm font-bold text-gray-600 hover:bg-primary/5 hover:text-primary transition-colors rounded-xl mx-1.5 group">
                          <FaUserCircle className="text-gray-400 group-hover:text-primary transition-colors text-lg" /> {t('header.admin_panel')}
                        </Link>
                        <Link to="/admin/settings" onClick={() => setIsUserDropdownOpen(false)} className="flex items-center gap-3 px-4 py-3 text-sm font-bold text-gray-600 hover:bg-primary/5 hover:text-primary transition-colors rounded-xl mx-1.5 group">
                          <FaCog className="text-gray-400 group-hover:text-primary transition-colors text-lg" /> {t('header.settings')}
                        </Link>
                      </>
                    )}
                    <div className="border-t border-gray-50 my-2"></div>
                    <button onClick={logoutHandler} className="flex items-center gap-3 px-4 py-3 text-sm font-bold text-red-500 hover:bg-red-50 w-[calc(100%-0.75rem)] text-start transition-colors focus:outline-none cursor-pointer rounded-xl mx-1.5 group">
                      <FaSignOutAlt className="text-lg rtl:rotate-180 text-red-400 group-hover:text-red-600 transition-colors" /> {t('header.logout')}
                    </button>
                  </div>
                </div>
              </div>
      ...
      <div className={`fixed top-0 inset-s-0 h-full w-[80vw] max-w-sm bg-white z-[110] shadow-2xl transition-transform duration-300 ease-out transform flex flex-col ${ isMobileMenuOpen ? 'translate-x-0' : 'ltr:-translate-x-full rtl:translate-x-full' }`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          <div>
            ...
            <div className="space-y-1">
              <button onClick={() => handleCategorySelect('')} className={`w-full text-start px-4 py-3 rounded-xl font-heading font-bold tracking-wide uppercase transition-colors focus:outline-none cursor-pointer ${ !currentCategoryId ? 'bg-primary/10 text-primary' : 'text-gray-600 hover:bg-primary/5 hover:text-primary' }`}>{t('header.all_gear')}</button>
              {displayCategories.map((cat) => (
                <button key={cat._id} onClick={() => handleCategorySelect(cat._id)} className={`w-full text-start px-4 py-3 rounded-xl font-heading font-bold tracking-wide uppercase transition-colors focus:outline-none cursor-pointer ${ currentCategoryId === cat._id ? 'bg-primary/10 text-primary' : 'text-gray-600 hover:bg-primary/5 hover:text-primary' }`}>{getDBText(cat.name)}</button>
              ))}
            </div>
          </div>
          {userInfo && (
            <div>
              ...
              <div className="space-y-1">
                <Link to="/profile" className="flex items-center gap-3 w-full px-4 py-3 rounded-xl text-gray-600 hover:bg-primary/5 hover:text-primary font-bold transition-colors"><FaUser className="text-gray-400" /> {t('header.my_account')}</Link>
                {userInfo.isAdmin && <Link to="/admin/dashboard" className="flex items-center gap-3 w-full px-4 py-3 rounded-xl text-gray-600 hover:bg-primary/5 hover:text-primary font-bold transition-colors"><FaUserCircle className="text-primary" /> {t('header.admin_panel')}</Link>}
              </div>
            </div>
          )}
        </div>
        ...
        <div className="p-4 border-t border-gray-100 shrink-0 bg-gray-50 space-y-3">
          <CustomSelect
            ...
            triggerClassName="!bg-white !border-gray-200 hover:!border-primary !py-3 !rounded-xl text-sm font-bold text-gray-600 shadow-sm"
            menuClassName="!bottom-full !top-auto !mb-2"
            renderValue={(opt) => (
