...
const ProductReviews = forwardRef(({ productId, productRating = 0, numReviews = 0, onReviewAdded }, ref) => {
      ...
      const submitReviewHandler = async (e) => {
            ...
            setReviewError('');
            if (rating === 0) {
                  setReviewError(lang === 'ar' ? 'الرجاء اختيار التقييم بالنجوم' : 'Please select a star rating');
            ...
            try {
                  ...
                  setReviewSuccess(lang === 'ar' ? 'تم تقديم التقييم بنجاح!' : 'Review submitted successfully!');
                  setRating(0);
                  setComment('');
                  ...
                  setReviewError(error.response?.data?.message || (lang === 'ar' ? 'فشل في الإرسال' : 'Failed to submit review'));
                  setReviewLoading(false);
            }
      };
            ...
            <div ref={ref} className="mt-16 pt-16 border-t border-gray-100 scroll-mt-24 animate-fade-in-up">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
                        <h2 className="text-3xl md:text-4xl font-extrabold text-dark tracking-tight text-start">
                              {lang === 'ar' ? 'آراء' : 'Customer'} <span className="text-primary">{lang === 'ar' ? 'العملاء' : 'Reviews'}</span>
                        </h2>

                        {numReviews > 0 && (
                              <div className="flex items-center gap-4 bg-gray-50 px-6 py-4 rounded-2xl border border-gray-100 shadow-sm shrink-0">
                                    ...
                                    <div className="flex flex-col text-start">
                                          ...
                                          <span className="text-sm font-bold text-gray-500 mt-1">
                                                {lang === 'ar' ? `بناءً على ${ numReviews } تقييم` : `Based on ${ numReviews } reviews`}
                                          </span>
                                    </div>
                              </div>
                        )}
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                        ...
                        <div className="lg:col-span-7 space-y-6">
                              {reviews.length === 0 ? (
                                    <div className="bg-gray-50 rounded-3xl p-10 text-center border border-gray-100">
                                          <p className="text-gray-500 font-bold text-lg mb-2">{lang === 'ar' ? 'لا توجد تقييمات بعد.' : 'No reviews yet.'}</p>
                                          <p className="text-sm text-gray-400">{lang === 'ar' ? 'كن أول من يشارك رأيه حول هذا المنتج!' : 'Be the first to share your thoughts about this product!'}</p>
                                    </div>
                              ) : (
                                    <>
                                          {reviews.map((review) => (
                        ...
                        <div className="lg:col-span-5 text-start">
                              <div className="bg-white rounded-3xl p-6 sm:p-8 border border-gray-100 shadow-xl shadow-gray-200/40 sticky top-24">
                                    <h3 className="text-xl font-extrabold text-dark mb-6 uppercase tracking-wide">{lang === 'ar' ? 'أضف تقييماً' : 'Write a Review'}</h3>

                                    {reviewError && <div className="mb-6 p-4 bg-red-50 text-red-600 border border-red-100 rounded-xl text-sm font-bold">{reviewError}</div>}
                                    {reviewSuccess && <div className="mb-6 p-4 bg-green-50 text-green-600 border border-green-100 rounded-xl text-sm font-bold">{reviewSuccess}</div>}

                                    {userInfo ? (
                                          <form onSubmit={submitReviewHandler} className="space-y-6">
                                                <div>
                                                      <label className="block text-sm font-bold text-gray-600 mb-3 uppercase tracking-wider">{lang === 'ar' ? 'التقييم العام' : 'Overall Rating'}</label>
                                                ...
                                                <div>
                                                      <label className="block text-sm font-bold text-gray-600 mb-3 uppercase tracking-wider">{lang === 'ar' ? 'تعليقك' : 'Your Comment'}</label>
                                                      <textarea
                                                            rows="4"
                                                            value={comment}
                                                            required
                                                            onChange={(e) => setComment(e.target.value)}
                                                            placeholder={lang === 'ar' ? 'شاركنا تجربتك ورأيك بالمنتج...' : 'Share your experience with this product...'}
                                          ...
                                          <div className="bg-gray-50 p-8 rounded-2xl border border-gray-100 text-center">
                                                ...
                                                <p className="text-dark font-bold mb-6">{lang === 'ar' ? 'يرجى تسجيل الدخول لمشاركة رأيك.' : 'Please log in to share your thoughts.'}</p>
